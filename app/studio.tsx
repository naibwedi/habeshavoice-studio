...
import { formatTime, LANGUAGES, type Language, type Transcript } from "../lib/types";
...
export default function Studio() {
  ...
  async function submitTranscription() {
    ...
    try {
      ...
      const uploaded=await upload("pending/"+crypto.randomUUID()+"."+extension,file,{access:"private",handleUploadUrl:"/api/upload",contentType:file.type||"application/octet-stream"});
      const data=await api<{transcript:Transcript}>("/api/transcribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audioUrl:uploaded.url,language,source:fileSource,fileName:file.name,mime:file.type||"application/octet-stream",consent:true})});
  ...
  async function save() {
    ...
    try {
      const d=await api<{transcript:Transcript}>("/api/transcripts/"+selected.id,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:selected.title,text:selected.text})});
      setSelected(d.transcript);setSaved(old=>[d.transcript,...old.filter(t=>t.id!==d.transcript.id)]);setDirty(false);notify("Changes saved.");
    }catch(e){setError((e as Error).message);}finally{setBusy(null);}
  }
  ...
  const row=(t:Transcript) => <button key={t.id} className={"session-row"+(selected?.id===t.id?" selected":"")} onClick={()=>changeDocument(t)}><span className="session-icon">{<AudioLines size={17}/>}</span><span><b>{t.title}</b><small>{LANGUAGES[t.language].name}<span>·</span>{formatTime(t.duration)}<span>·</span>{new Date(t.createdAt).toLocaleDateString(undefined,{month:"short",day:"numeric"})}</small></span><ChevronRight size={16}/></button>;
  return <div className="shell" inert={!hydrated} aria-busy={!hydrated}>
    ...
    {view==="library"?<><label className="searchbar"><Search size={18}/><input aria-label="Search transcripts" placeholder="Search your transcripts…" value={search} onChange={e=>setSearch(e.target.value)}/></label><section className="panel library-list">{filtered.length?filtered.map(row):<div className="empty"><FolderOpen size={32}/><h3>{search?"No matches yet":"Your library starts here"}</h3><p>{search?"Try another title, word, or language.":"Transcribe your first recording to keep your words here."}</p><button className="button" onClick={()=>setView("studio")}>Open studio<ArrowRight size={15}/></button></div>}</section></>:
    ...
    <section className="panel transcript-panel" aria-label="Transcript editor"><div className="panel-head"><h2 className="panel-title"><FileText size={17}/>Your transcript</h2><div className="transcript-tools"><button className="icon-button" aria-label="Copy transcript" disabled={!selected} onClick={()=>void copy()}><Copy size={15}/></button><button className="icon-button" aria-label="Export transcript" disabled={!selected} onClick={()=>setModal("export")}><ArrowDownToLine size={16}/></button>{selected&&<button className="icon-button" aria-label="Delete transcript" onClick={()=>setModal("delete")}><Trash2 size={15}/></button>}</div></div>
    {selected?<><div className="document"><div className="document-meta"><span className="badge">{LANGUAGES[selected.language].native}</span><span>{formatTime(selected.duration)+" AUDIO"}</span></div><input className="document-title" aria-label="Transcript title" disabled={!!busy} maxLength={120} value={selected.title} onChange={e=>{setSelected({...selected,title:e.target.value});setDirty(true);}}/><p className="document-subtitle">Listen back, check the details, and make your final edits.</p>
    ...
    </div><div className="doc-bottom"><span>{wordCount} words · {dirty?"Unsaved changes":"Saved to library"}</span><button className="button" disabled={!!busy||!dirty} onClick={()=>void save()}>{busy==="save"?<LoaderCircle size={15} className="loading-spin"/>:<Save size={15}/>} Save changes</button></div></>:<div className="empty" style={{flex:1}}><AudioLines size={39} strokeWidth={1}/><h3>Every story starts with a voice.</h3><p>Record or upload a short clip. Your words will have a home right here.</p><button className="button" onClick={()=>setCapture("upload")}><CloudUpload size={15}/>Upload audio</button></div>}</section></div>}
